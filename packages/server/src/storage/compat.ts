...
type PolicyStorageSource =
  | PolicyStorage
  | Pick<AuthStorage, "policies">
  | unknown;
...
type D1DatabaseLike = {
  ...
  batch?<T = unknown>(
    statements: Array<ReturnType<D1PreparedStatementLike["bind"]>>,
  ): Promise<T>;
};
...
export function resolvePolicyStorage(
  source: PolicyStorageSource,
): PolicyStorage {
...
export function resolveContextStorage(c: Context<AppEnv>): AuthStorage {
  ...
  if (!storage) {
    throw new Error(
      "storage not set in context — ensure createApp() receives a storage adapter",
    );
  }
  return storage;
}
...
function createD1AuditStorage(db: D1DatabaseLike): AuditStorage {
  return {
    ...
    async getActionCounts(_orgId: string, _query: DashboardAuditQuery) {
      throw new Error(
        "D1 audit storage adapter does not support getActionCounts()",
      );
    },

    async writeIdentitySuspendedEvent(
      _identity: StoredIdentity,
      _reason: string,
      _actorId: string,
    ): Promise<void> {
      throw new Error(
        "D1 audit storage adapter does not support writeIdentitySuspendedEvent()",
      );
    },
  };
}
